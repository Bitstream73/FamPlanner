...
## Objective
Build the Express server with security middleware, Socket.IO, request logging, and error handling.

## Plan First
Propose your middleware stack order and error handling strategy. Wait for approval.

## Steps

### 1. Expand `src/index.js`
- Initialize Express with full middleware stack
- Log startup: `logger.info('system', 'startup', { version, nodeVersion, env })`
- Configure: helmet, cors, express-rate-limit, JSON body parser, cookie-parser (with SESSION_SECRET), static files
- Set up Socket.IO
- Register route placeholders (to be filled in Phase 4A and Phase 6)
- Health check endpoint with service status
- Graceful shutdown: `logger.info('system', 'shutdown_initiated', { signal })`
- Export `createApp()` for testing

### 2. `src/middleware/errorHandler.js`
- Export `errorHandler` middleware and `AppError` class
- Log all errors: `logger.error('api', 'unhandled_error', { path, method, requestId }, error.message)`
- Include stack trace only in development
- Return sanitized JSON error responses
- Custom status codes via `AppError`
...
- Generate UUID `requestId` per request, attach to `req.requestId`
- Log request start (method, path, IP)
- Log completion (status code, duration)
- Log slow requests (>1000ms) as warnings
- Skip verbose logging for `/api/health`

### 5. `src/middleware/logContext.js`
- Create child logger per request with `requestId`
- Attach to `req.logger` for use in route handlers
...
## Checkpoint
```bash
git add -A && git commit -m "phase-4: express server, middleware, error handling" && git push
```

Report: "Phase 4 complete. N/N tests passing. Server boots cleanly. Ready for /phase-4a."
