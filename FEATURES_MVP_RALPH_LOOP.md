...
**OVERALL DESCRIPTION:** FamPlanner takes ideas from the business management app, Quinyx, and conforms and improves upon them for a family context. If there is not enough description of a feature for you to implement it fallback to your know of similar feature in Quinyx. If that isn't enough information, stop and ask for information from the user.

Family OS (Quinyx reimagined) — Requirements Checklist
MVP (ship fast: “shared plan + chores that actually work”)

**Accounts & roles**

 Household creation + invite via link/SMS/email

 Roles: Parent/Guardian, Teen, Kid (limited), Caregiver (temporary)

 Basic profile: name, avatar, pronouns optional

 Permission rules: who can assign tasks, edit calendar, view notes

**Calendar & coverage**

 Shared family calendar (day/week/month)

 Create events with: title, location text, start/end, notes

 Assign “responsible person” to an event (driver/chaperone)

 Availability blocks (“unavailable” windows)

 Conflict detection (overlap + “no responsible person” warnings)

 Simple recurring events (weekly)

**Chores & tasks**

 Task types: one-time and recurring

 Assign to person(s) + due date + reminders

 Task checklist (sub-steps)

 Time estimate + difficulty tag

 “Done” confirmation + optional note/photo upload

 Rotations (round-robin assignee for recurring chores)

 Task views: Today, Upcoming, Overdue

**Routines**

 Routine builder (Morning/Evening/Leaving house)

 Run routine as a checklist (tap-to-complete)

 “Auto-reset daily” option

**Communication**

 Household announcements feed

 Comment thread on tasks/events

 @mentions + push notifications

 Quiet hours per user

**Household handbook (lightweight)**

 Pin notes (Wi-Fi, trash days, emergency contacts)

 Basic “How-to” entries (title + steps + images)

**Notifications**

 Push + email optional

 Reminder rules: on due, 1 hour before, daily digest

**Security & basics**

 Data encryption at rest + in transit

 Export basic data (CSV for tasks/events) or at least “download my data”

 Audit log (minimal): who edited/deleted tasks/events
